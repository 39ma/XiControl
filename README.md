...
- ⚡ **Режимы производительности**: Тихий / Авто / Турбо / Полная мощность.
...
| Mi-кнопка, короткое нажатие | Следующий режим производительности + OSD |
...
модели он гасит подсветку клавиатуры и снижает яркость экрана — самый экономный
профиль). Если он не нужен, его можно убрать из меню, панели и цикла Mi-кнопки —
в `%APPDATA%\XiControl\config.json`:

```json
"EcoMode": false
```

По умолчанию включён. После правки перезапусти приложение.
...
- Порог «беречь батарею» (~80%) зашит в прошивку — произвольный процент через WMI невозможен.
