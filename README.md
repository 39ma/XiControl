...
- 🔌 **Профили питания** — свой режим производительности для сети и батареи:
  подключил зарядку → «от сети» (например, Турбо), отключил → «от батареи»
  (например, Тихий). Плюс **память яркости**: яркость запоминается отдельно для
  каждого источника и восстанавливается при смене питания. Работает после сна
  и перезагрузки.
...
**Настройки → «Режим при старте»** (два взаимоисключающих переключателя):
